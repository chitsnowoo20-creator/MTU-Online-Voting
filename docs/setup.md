...
Everything needed to run Campus Elections locally or against a fresh Supabase
project. For what the system is and what it guarantees, see the
[README](../README.md).
...
```bash
git clone https://github.com/chitsnowoo20-creator/MTU-Online-Voting.git
```

```bash
cd MTU-Online-Voting && pnpm install
```
